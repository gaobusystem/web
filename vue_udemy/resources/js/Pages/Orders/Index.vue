        ...
        <div class="py-12">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-2">
                <div class="mt-3 mb-3 ml-3 flex">
                    ...
                    <span v-if="props.orders.data.length===0" class="m-2">該当する顧客はありません。</span>
                </div>
                ...
                <tr v-for="order in orders.data" :key="order.id">
                    <td class="border border-gray-400 px-4 py-2 text-center">{{ order.id }}</td>
                    <td class="border border-gray-400 px-2 py-2 text-center text-xs">{{ order.orderday }}</td>
                    <td class="border border-gray-400 px-4 py-2">{{ order.customer.name }}</td>
                    <td class="border border-gray-400 px-2 py-2 text-center">{{ order.product1.name }}</td>
                    <td class="border border-gray-400 px-2 py-2 text-center">{{ order.product1.price }}</td>
                    <td class="border border-gray-400 px-2 py-2 text-center">{{ order.product1.tax }}%</td>
                    <td class="border border-gray-400 px-2 py-2 text-center">{{ order.num1 }}</td>
                    <td class="border border-gray-400 px-2 py-2 text-center">{{ order.product2?.name }}</td>
                    <td class="border border-gray-400 px-2 py-2 text-center">{{ order.product2?.price }}</td>
                    <td class="border border-gray-400 px-2 py-2 text-center">{{ order.product2?.tax }}{{ order.product2 ? '%' : '' }}</td>
                    <td class="border border-gray-400 px-2 py-2 text-center">{{ order.num2 }}</td>
                    <td class="border border-gray-400 px-2 py-2 text-center">{{ order.product3?.name }}</td>
                    <td class="border border-gray-400 px-2 py-2 text-center">{{ order.product3?.price }}</td>
                    <td class="border border-gray-400 px-2 py-2 text-center">{{ order.product3?.tax }}{{ order.product3 ? '%' : '' }}</td>
                    <td class="border border-gray-400 px-2 py-2 text-center">{{ order.num3 }}</td>
                     <td class="border border-gray-400 px-4 py-2 text-center">
                        <Link :href="route('orders.show', { order: order.id, page: orders.meta.current_page, search_str: form.search_str })"
...
<nav class="flex items-center gap-x-1 mt-2 mb-2" aria-label="Pagination">
  <div class="flex items-center gap-x-1">
    <div v-for="(link,index) in orders.meta.links">
        <div v-if="index==0">
            <Link :href="route('orders.index', { page: orders.meta.current_page-1, search_str: form.search_str })"
        ...
        <div v-else>
            <Link :href="route('orders.index', { page: link['label'], search_str: form.search_str })"
